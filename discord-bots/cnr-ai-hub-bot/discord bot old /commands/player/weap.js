...
// Mapping weapon IDs to their corresponding role IDs
const weaponData = {
    ...
    22: { name: "MP5", damage: 40, price: 4000, roleId: '1249710333120086168' },
    ...
    42: { name: "Remington 700", damage: 90, price: 9000, roleId: '1249710336878317639' },
...
module.exports = {
    name: 'weap', // Command name
    description: '🔫 Buy a weapon using its ID',

    async execute(message, args) {
        const weaponId = parseInt(args[0], 10); // Get weapon ID from the first argument

        if (!weaponId) {
            return message.reply('Please provide a valid weapon ID.');
        }

        const weapon = weaponData[weaponId];

        if (!weapon) {
            return message.reply('⚠️ Invalid weapon ID.');
        }
        ...
        try {
            ...
            return message.reply('⚠️ Error retrieving your profile. Please try again later.');
        }

        // Check if the user has enough currency
        if (profile.currency < weapon.price) {
            return message.reply(`⚠️ You don't have enough currency to buy the ${weapon.name}. You need $${weapon.price}, but you only have $${profile.currency}.`);
        }
        ...
        try {
            ...
            return message.reply('⚠️ Error saving your profile. Please try again later.');
        }

        // Assign the weapon role to the user
        const guild = message.guild; // Get the guild (server) where the command was executed
        const roleId = weapon.roleId; // Get the role ID associated with the weapon

        try {
            const member = await guild.members.fetch(userId); // Fetch the member object for the user
            const role = guild.roles.cache.get(roleId); // Get the role from the guild

            if (role) {
                await member.roles.add(role); // Add the role to the user
            } else {
                console.error('Role not found:', roleId);
                return message.reply('⚠️ Error: Role not found for this weapon.');
            }
        } catch (error) {
            console.error('Error adding role:', error);
            return message.reply('⚠️ Error assigning the weapon role. Please try again later.');
        }
