...
const { getOrCreateUserProfile } = require('../../utils/profileManager'); // Adjust the path if necessary

module.exports = {
    name: 'plantc4',
    description: 'Plant a C4 explosive',
    ...
    async execute(message) {
        ...
        if (member.roles.cache.has(roles.cop)) {
            return message.reply("Cops can't use C4 Bombs.");
        }

        // Get user profile to check inventory
        const userProfile = getOrCreateUserProfile(member.id);
        
        // Check if user has C4 in inventory
        if (!userProfile.inventory.includes('c4')) {
            return message.reply('You do not have a C4 in your inventory.');
        }
        ...
        if (!interiorName) {
            return message.reply('You are not in a valid interior.');
        }

        // Embed message for planting C4
        const embedMessage = new EmbedBuilder()
            .setTitle('You planted a C4 explosive!')
            .setDescription(`This command was executed by ${message.author.username}. It will detonate in 30 seconds.`)
            .setColor('#8B0000')
            .setImage('https://tenor.com/view/csgo-bomb-animation-csgo.gif');

        await message.reply({ embeds: [embedMessage] });
        await message.channel.send(`EVERYONE RUN FROM THE ${interiorName} OR EVERYONE GONNA DIE IN 30 SECONDS!`);

        // Detonation logic after 30 seconds
        setTimeout(async () => {
            const membersInInterior = message.guild.members.cache.filter(m =>
            ...
            for (const targetMember of membersInInterior.values()) {
                ...
                await message.channel.send(`${targetMember.user.username} is caught in the explosion and is now dead!`);
            }
            ...
            await message.channel.send({ embeds: [detonateMessage] });
        }, 30000);
    },
};
