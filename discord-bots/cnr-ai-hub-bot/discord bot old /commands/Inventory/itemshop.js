...
module.exports = {
    name: 'itemshop',
    description: 'Access the item shop to purchase exclusive items!',
    ...
    async execute(message) {
        ...
        const messageReply = await message.channel.send({ embeds: [shopEmbed], components: [row] });

        const filter = (buttonInteraction) => buttonInteraction.user.id === user.id;
        const collector = messageReply.createMessageComponentCollector({ filter, time: 120000 });
        ...
        collector.on('end', collected => {
            if (collected.size === 0) {
                messageReply.delete();
                message.channel.send('You did not make a selection in time. Please try again.');
            }
        });
    },
};
