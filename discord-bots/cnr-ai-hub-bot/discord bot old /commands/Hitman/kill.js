...
module.exports = {
    name: 'kill',
    description: 'Execute a hit on a marked target',
    async execute(message, args) {
        const hitmanRoleId = '1248697926515953694';
        const targetRoleId = '1265726365597044837';
        const deadRoleId = '1247164748278140978';

        const hitman = message.member;

        // Check if there is a target mentioned
        const target = message.mentions.members.first();
        if (!target) {
            const errorMessage = new EmbedBuilder()
                .setTitle('🎯 No Target Specified')
        ...
        if (!hitman.roles.cache.has(hitmanRoleId)) {
            const message = new EmbedBuilder()
                ...
                .setColor('Red');
            return message.reply({ embeds: [message] });
        }
        ...
        if (!target.roles.cache.has(targetRoleId)) {
            const message = new EmbedBuilder()
                ...
                .setDescription(`You can only kill targets with the ${message.guild.roles.cache.get(targetRoleId).name} role, ${hitman.user.username}.`)
                .setColor('Red');
            return message.reply({ embeds: [message] });
        }

        // Execute the hit
        const payment = 3000; // Assuming a fixed payment of 3000 coins
        ...
        const killMessage = new EmbedBuilder()
            ...
            .setColor('Green');
        await message.channel.send({ embeds: [killMessage] });
