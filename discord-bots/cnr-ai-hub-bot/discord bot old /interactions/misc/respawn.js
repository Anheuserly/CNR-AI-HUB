...
module.exports = {
    data: new SlashCommandBuilder()
        .setName('respawn')
        .setDescription('Respawn after death.'),
    async execute(interaction) {
        const deadRoleId = '1247164748278140978';

        // Check if the user has the dead role
        if (!interaction.member.roles.cache.has(deadRoleId)) {
            await interaction.reply("⛔ You are not dead. You cannot use this command.");
            return;
        }

        await interaction.reply("You are dead. Waiting for respawn...");
        ...
        // Revoke dead role
        const deadRole = interaction.guild.roles.cache.get(deadRoleId);
        if (deadRole) {
            await interaction.member.roles.remove(deadRole);
        }
    },
};
